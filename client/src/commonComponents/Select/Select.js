...
const Select = (props) => {
    ...
    useEffect(() => {
        if(selectValue) {
            if (multiple || Array.isArray(selectValue)) {
                const valuesSelected = optionsChildren.reduce((acc, curr) => {
                    const { value, children, label, optionIndex } = curr.props;

                    if(selectValue.includes(value)) {
                        acc[0].push((typeof children === 'undefined' ? 'undefined' : typeof children === 'object' ? label : children));
    ...
    const filterOptions = options => {
        const filteredOptions = options.filter(({ props: { children, label, value}}) => {
            ...
            const option = typeof value === 'undefined' ? 'undefined' : typeof value === 'object' ? label : children;
    ...
    const handleClickSelectAll = () => {
        if (selectValue.length) {
          ...
          const selectedOptions = optionsChildren.reduce((acc,
              { props: { value, children, label, optionIndex, disabled } }
            ) => {
              if (!disabled) {
    ...
    const handleClickSearchCheckbox = () => {
        const selectedOptions = optionsChildrenSorted.reduce((acc,
            { props: { value, children, label, optionIndex, disabled } }
          ) => {
            if (disabled) {
              acc.disabledOptions = acc.disabledOptions ? [...acc.disabledOptions, value] : [value];
            } else {
              const valueShowed = typeof children === 'object' ? label : children;
              acc.selectValue = acc.selectValue ? [...acc.selectValue, value] : [value];
              acc.displayedValue = acc.displayedValue? [...acc.displayedValue, valueShowed]: [valueShowed];
        ...
        const prevValuesSelected = selectValue && selectValue.filter(value => !selectedOptions.selectValue.includes(value));
    
        const prevDisplayedSelected = displayedValue && displayedValue.filter(value => !selectedOptions.displayedValue.includes(value));
        ...
        <SelectProvider value={{
            ...
            >
                <button ref={selectTriggerRef} 
                    ...
                    >
                        ...
                        {!multiple && filter && displayedValue && displayedValue.length > 0 && (
                            <IconButton className="cc-select-clean-value" onClick={handleCleanValue}>
                ...
                <div className='cc-select-wrapper-options' ref={wrapperOptionsRef} style={styles.popper} {...attributes?.popper}>
                    ...
                    {filter && <SelectSearch />}
                    <div id={formFieldContext.id + '-listbox'} tabIndex='-1' className='cc-select-options' style={{ maxHeight: visibleOptions * 3 + 'rem'}}>
